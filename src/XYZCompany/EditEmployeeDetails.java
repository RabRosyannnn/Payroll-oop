...
public class EditEmployeeDetails extends javax.swing.JInternalFrame {
   ...
   public void DeleteAction() {
    ...
    try {
        ...
        // Delete from the dependent table(s) first
        PreparedStatement stat1 = con.prepareStatement("DELETE FROM fixedsalarytable WHERE EmployeeID = ?");
        stat1.setString(1, EmpID);
        stat1.executeUpdate();

        PreparedStatement stat2 = con.prepareStatement("DELETE FROM hourlysalarytable WHERE EmployeeID = ?");
        stat2.setString(1, EmpID);
        stat2.executeUpdate();

        // Now delete from the EmployeeTable
        PreparedStatement stat3 = con.prepareStatement("DELETE FROM EmployeeTable WHERE EmployeeID = ?");
        stat3.setString(1, EmpID);
        int row = stat3.executeUpdate();
        ...
        if (Success) {
            ...
            JOptionPane.showMessageDialog(rootPane, "Employee record has been deleted", "Employee Deletion Successful", JOptionPane.INFORMATION_MESSAGE);
        } else {
            con.rollback(); // Rollback the transaction if no rows affected
        }
        ...
        JOptionPane.showMessageDialog(rootPane, "A Database exception error occurred during deletion\nError: " + sqlE, "Database Exception Error", JOptionPane.ERROR_MESSAGE);
    } finally {
        try {
    ...
    private void initComponents() {
        ...
        btnDelete.setIcon(new javax.swing.ImageIcon(getClass().getResource("/XYZCompany/SaveAndExitIcon.png"))); // NOI18N
